...
def size():
    sizes = monitor_areas()
    optimal_geometry = [min([s[0] for s in sizes]), 0, max([s[2] for s in sizes]) - min([s[0] for s in sizes]),
                        max([s[3] for s in sizes])]
    res = f"{optimal_geometry[2]}x{optimal_geometry[3]}+{optimal_geometry[0]}+{optimal_geometry[1]}"
